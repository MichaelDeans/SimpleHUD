...
if config.enablePriorityStatus then
    TriggerEvent("chat:addSuggestion", "/pstart", "Start a priority.")
    TriggerEvent("chat:addSuggestion", "/pend", "Stop an active priority.")
    TriggerEvent("chat:addSuggestion", "/pcd", "Start a cooldown on priorities.", {
        {name="Time", help="Time in minutes to start a cooldown"}
    })
    TriggerEvent("chat:addSuggestion", "/pjoin", "Join the current priority.")
...
CreateThread(function()
    while true do
        ...
        if not hidden then
            ...
            if config.enablePriorityStatus then
                text(priorityText, 0.180, 0.911, 0.40, 4)
            end
            if config.enablePostals and nearestPostal and nearestPostal.code then
                text("~s~Nearby Postal: ~c~(" .. nearestPostal.code .. ")", 0.180, 0.912, 0.40, 4)
            end
            text("~c~" .. time .. " ~s~" .. zoneName, 0.180, 0.96, 0.40, 4)
            text("~c~| ~s~" .. compass .. " ~c~| " .. nearestPostal.code .. " ~s~" .. streetName, 0.180, 0.932, 0.55, 4)
        end
